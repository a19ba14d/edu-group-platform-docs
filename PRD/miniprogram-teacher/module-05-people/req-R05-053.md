# 【R05-053】推送班主任
<a id="R05-053"></a>

- **Requirement ID**：【R05-053】
- **Feature name**：推送班主任
- **Source**：模块05：人员相关 15% > 学生管理 > 学生信息 > 补充功能 > 健康 > 体温检测 > 报警信息推送 > 推送班主任
- **Applicable Platform/Entry**：小程序（教职工/教师端） / 模块05 / 推送班主任
- **User Roles & Permission Requirements**：
  - 可执行角色：教职工（教师/班主任/行政等）
  - 权限点（最小粒度）：`req:R05-053`
  -（可选）通用权限点：`people:send`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为教职工（教师/班主任/行政等），我希望能够完成“推送班主任”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为教职工（教师/班主任/行政等），我希望选择通道/模板并发送通知，且能跟踪发送状态、失败原因与（如需）撤回。
  - 场景 3：作为移动端用户，我希望在弱网/重复点击时系统能避免重复提交并给出明确提示。
- **Business Rules & Boundaries**：
  - 输入校验：所有 UUID 入参必须校验格式；非法输入应返回可定位的错误。
  - 通知发送必须可追踪状态（成功/失败/待发送/已撤回等）；失败原因可定位；（如需求要求）支持延迟/撤回。
- **Data Scope（school_id）**：
  - 仅允许访问与当前用户身份绑定的 `school_id` 数据（不可跨校）。
  - 仅允许访问与本人/授权关系相关的数据（学生本人、监护关系、任教/岗位范围等）。
- **Audit Log Requirements**：
  - Recommended：登录/绑定/信息修改/提交等关键行为建议记录操作日志（至少包含 operator_id、school_id、action、resource_type/id、timestamp）。
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-NOTIFY`
  - 验收用例：
    1. 权限校验：具备 `req:R05-053`（及必要通用权限点）才允许执行；无权限必须拒绝。
    2. 租户隔离：尝试跨 school_id（或未授权 X-View-Scope）访问/操作必须失败；不得出现跨校可查/可改。
    3. 输入校验：UUID/必填字段/格式校验失败时应返回明确错误。
    4. 发送可追踪：发送记录可查询状态（成功/失败/待发送/已撤回等）；失败可重试且记录原因。

## 相关需求（See also）
- [【R05-024】班主任修改](./req-R05-024.md)
- [【R05-020】审批/驳回](./req-R05-020.md)
- [【R05-054】推送家长](../../miniprogram-parent/module-05-people/req-R05-054.md)

## 导航
- 返回模块 README：[./README.md](./README.md)
- 返回平台 README：[../README.md](../README.md)
- PRD 总入口：[../../README.md](../../README.md)
- 追溯矩阵：[../../traceability-matrix.md](../../traceability-matrix.md)
