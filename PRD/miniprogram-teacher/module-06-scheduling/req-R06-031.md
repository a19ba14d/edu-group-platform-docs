...
- **Requirement ID**：【R06-031】
- **Feature name**：教师
- **Source**：模块06：排课系统 15% > 展示体系 > 教师
- **Applicable Platform/Entry**：小程序（教职工/教师端） / 模块06 / 教师
- **User Roles & Permission Requirements**：
  - 可执行角色：教职工（教师/班主任/行政等）
  - 权限点（最小粒度）：`req:R06-031`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为教职工（教师/班主任/行政等），我希望能够完成“教师”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为教职工（教师/班主任/行政等），我希望设置关键约束并生成/调整排课结果，系统能避免教师/班级/教室冲突。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-GENERAL`
  - 验收用例：
    1. 权限校验：具备 `req:R06-031`（及必要通用权限点）才允许执行；无权限必须拒绝。
...
- [【R06-029】整表排课工具](../../web-admin/module-06-scheduling/req-R06-029.md)
