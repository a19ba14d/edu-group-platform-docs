...
- **Requirement ID**：【R08-021】
- **Feature name**：会议室申请撤消
- **Source**：模块08：财物相关  15% > 资产管理 > 会议室管理 > 会议室申请撤消
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块08 / 会议室申请撤消
- **User Roles & Permission Requirements**：
  - 可执行角色：财务管理员、资产管理员、学校管理员（School）、集团管理员（Group，进入学校视角）
  - 权限点（最小粒度）：`req:R08-021`
  -（可选）通用权限点：`finance:create`, `finance:update`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为财务管理员，我希望能够完成“会议室申请撤消”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Audit Log Requirements**：
  ...
  - Required：管理端关键操作应覆盖：CREATE（新增/发送/导入等）、UPDATE（编辑/审批/设置/调整）。
...
- **Acceptance Criteria（verifiable）**：
  ...
  - 验收用例：
    1. 权限校验：具备 `req:R08-021`（及必要通用权限点）才允许执行；无权限必须拒绝。
    ...
    8. 删除语义：默认软删；删除后列表不可见；详情访问按规则拒绝或标记已删除。
