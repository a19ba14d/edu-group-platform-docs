...
- **Requirement ID**：【R08-020】
- **Feature name**：通知
- **Source**：模块08：财物相关  15% > 资产管理 > 会议室管理 > 会议室申请 > 审核 > 通知
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块08 / 通知
- **User Roles & Permission Requirements**：
  - 可执行角色：财务管理员、资产管理员、学校管理员（School）、集团管理员（Group，进入学校视角）
  - 权限点（最小粒度）：`req:R08-020`
  -（可选）通用权限点：`finance:send`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为财务管理员，我希望能够完成“通知”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为财务管理员，我希望选择通道/模板并发送通知，且能跟踪发送状态、失败原因与（如需）撤回。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-NOTIFY`
  - 验收用例：
    1. 权限校验：具备 `req:R08-020`（及必要通用权限点）才允许执行；无权限必须拒绝。
    ...
    8. 发送可追踪：发送记录可查询状态（成功/失败/待发送/已撤回等）；失败可重试且记录原因。
