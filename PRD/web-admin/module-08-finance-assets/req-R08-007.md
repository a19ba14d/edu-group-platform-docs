...
- **Requirement ID**：【R08-007】
- **Feature name**：导出报表
- **Source**：模块08：财物相关  15% > 资产管理 > 资产列表 > 统计报表 > 导出报表
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块08 / 导出报表
- **User Roles & Permission Requirements**：
  - 可执行角色：财务管理员、资产管理员、学校管理员（School）、集团管理员（Group，进入学校视角）
  - 权限点（最小粒度）：`req:R08-007`
  -（可选）通用权限点：`finance:export`, `finance:report`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为财务管理员，我希望能够完成“导出报表”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为财务管理员，我希望导出/打印当前查询条件下的数据，并确保导出内容与权限/范围一致。
...
- **Acceptance Criteria（verifiable）**：
  ...
  - 验收用例：
    ...
    7. 导出一致性：导出数据与查询条件一致；具备导出权限才可执行；导出行为可追溯。
    8. 指标口径：同一指标在不同入口/视角下口径一致；结果可复现。
    9. 视角授权：集团汇总遵循 `X-View-Scope` 与授权范围；不得泄露未授权学校明细。
    10. 性能门禁：复杂报表满足附件3性能指标；明细列表分页且 `limit<=100`。
