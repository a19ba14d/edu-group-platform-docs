...
- **Requirement ID**：【R05-036】
- **Feature name**：差异在校列表
- **Source**：模块05：人员相关 15% > 学生管理 > 学生信息 > 差异 > 差异在校列表
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块05 / 差异在校列表
- **User Roles & Permission Requirements**：
  - 可执行角色：人事管理员、教务管理员、校区/部门管理员、学校管理员（School）、集团管理员（Group，进入学校视角）
  - 权限点（最小粒度）：`req:R05-036`
  -（可选）通用权限点：`people:read`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为人事管理员，我希望能够完成“差异在校列表”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-LIST`
  - 验收用例：
    1. 权限校验：具备 `req:R05-036`（及必要通用权限点）才允许执行；无权限必须拒绝。
