...
- **Requirement ID**：【R04-011】
- **Feature name**：增、删、改、查
- **Source**：模块04：校园体系 > 教室管理 > 增、删、改、查
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块04 / 增、删、改、查
- **User Roles & Permission Requirements**：
  - 可执行角色：学校管理员（School）、集团管理员（Group，进入学校视角）
  - 权限点（最小粒度）：`req:R04-011`
  -（可选）通用权限点：`campus:manage`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为学校管理员（School），我希望能够完成“增、删、改、查”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为学校管理员（School），我希望该功能具备明确输入输出与错误处理，便于验收复现。
...
- **Audit Log Requirements**：
  ...
  - Required：管理端关键操作应覆盖：VIEW（列表/详情）、CREATE（新增/发送/导入等）、UPDATE（编辑/审批/设置/调整）、DELETE（删除/撤消/屏蔽）。
  - 字段：operator_id/operator_name/target_school_id/action/resource_type/resource_id/trace_id/timestamp。
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-CRUD`
  - 验收用例：
    1. 权限校验：具备 `req:R04-011`（及必要通用权限点）才允许执行；无权限必须拒绝。
    ...
    10. 删除语义：默认软删；删除后列表不可见；详情访问按规则拒绝或标记已删除。
