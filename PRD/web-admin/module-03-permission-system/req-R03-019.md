...
- **Requirement ID**：【R03-019】
- **Feature name**：微信小程序
- **Source**：模块03：权限系统 > 岗位管理 > 学生（独立子系统） > 微信小程序
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块03 / 微信小程序
- **User Roles & Permission Requirements**：
  - 可执行角色：集团管理员（Group）、学校管理员（School）
  - 权限点（最小粒度）：`req:R03-019`
  -（可选）通用权限点：`permission:send`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为集团管理员（Group），我希望能够完成“微信小程序”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为集团管理员（Group），我希望选择通道/模板并发送通知，且能跟踪发送状态、失败原因与（如需）撤回。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-NOTIFY`
  - 验收用例：
    1. 权限校验：具备 `req:R03-019`（及必要通用权限点）才允许执行；无权限必须拒绝。
