...
- **Requirement ID**：【R03-009】
- **Feature name**：自动菜单
- **Source**：模块03：权限系统 > 管理权限 > RBAC > 菜单管理 > 自动菜单
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块03 / 自动菜单
- **User Roles & Permission Requirements**：
  - 可执行角色：集团管理员（Group）、学校管理员（School）
  - 权限点（最小粒度）：`req:R03-009`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为集团管理员（Group），我希望能够完成“自动菜单”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-GENERAL`
  - 验收用例：
    1. 权限校验：具备 `req:R03-009`（及必要通用权限点）才允许执行；无权限必须拒绝。
