...
- **Requirement ID**：【R01-003】
- **Feature name**：编辑用户
- **Source**：模块01：用户数据 > 用户注册 > 内部用户（管理账号） > 编辑用户
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块01 / 编辑用户
- **User Roles & Permission Requirements**：
  - 可执行角色：学校管理员（School）、集团管理员（Group，进入学校视角）
  - 权限点（最小粒度）：`req:R01-003`
  -（可选）通用权限点：`user:update`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为学校管理员（School），我希望能够完成“编辑用户”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-UPDATE`
  - 验收用例：
    1. 权限校验：具备 `req:R01-003`（及必要通用权限点）才允许执行；无权限必须拒绝。
