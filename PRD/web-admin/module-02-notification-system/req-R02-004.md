...
- **Requirement ID**：【R02-004】
- **Feature name**：短信列表
- **Source**：模块02：通知系统 > 短信下发 > 短信列表
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块02 / 短信列表
- **User Roles & Permission Requirements**：
  - 可执行角色：通知管理员、学校管理员（School）、集团管理员（Group，进入学校视角）
  - 权限点（最小粒度）：`req:R02-004`
  -（可选）通用权限点：`notify:read`, `notify:send`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为通知管理员，我希望能够完成“短信列表”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  ...
  - 验收用例：
    1. 权限校验：具备 `req:R02-004`（及必要通用权限点）才允许执行；无权限必须拒绝。
