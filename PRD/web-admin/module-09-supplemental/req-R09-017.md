...
- **Requirement ID**：【R09-017】
- **Feature name**：增加
- **Source**：模块09：补齐功能点 15% > 数据中心 > 基础数据管理 > 日志数据 > 异常操作信息 > 异常规则 > 增加
...
- **User Stories**：
  ...
  - 场景 2：作为系统管理员/运维管理员，我希望按表单要求填写信息并提交创建，系统校验通过后返回成功结果。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-CREATE`
  - 验收用例：
    1. 权限校验：具备 `req:R09-017`（及必要通用权限点）才允许执行；无权限必须拒绝。
