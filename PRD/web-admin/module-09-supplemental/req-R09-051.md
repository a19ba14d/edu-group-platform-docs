...
- **Requirement ID**：【R09-051】
- **Feature name**：按查询结果打印
- **Source**：模块09：补齐功能点 15% > 成绩评价系统 > 打印 > 按查询结果打印
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块09 / 按查询结果打印
- **User Roles & Permission Requirements**：
  - 可执行角色：系统管理员/运维管理员、集团管理员（Group）、学校管理员（School）
  - 权限点（最小粒度）：`req:R09-051`
  -（可选）通用权限点：`datacenter:read`, `datacenter:export`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为系统管理员/运维管理员，我希望能够完成“按查询结果打印”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为系统管理员/运维管理员，我希望导出/打印当前查询条件下的数据，并确保导出内容与权限/范围一致。
...
- **Audit Log Requirements**：
  ...
  - Required：管理端关键操作应覆盖：VIEW（列表/详情）、EXPORT/PRINT。
  - 字段：operator_id/operator_name/target_school_id/action/resource_type/resource_id/trace_id/timestamp。
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-EXPORT,T-LIST`
  - 验收用例：
    1. 权限校验：具备 `req:R09-051`（及必要通用权限点）才允许执行；无权限必须拒绝。
