...
- **Requirement ID**：【R09-040】
- **Feature name**：列表
- **Source**：模块09：补齐功能点 15% > 校园体系 > 教室绑定 > 临时教室绑定 > 列表
- **Applicable Platform/Entry**：Web 管理端（集团/学校双视角） / 模块09 / 列表
- **User Roles & Permission Requirements**：
  - 可执行角色：系统管理员/运维管理员、集团管理员（Group）、学校管理员（School）
  - 权限点（最小粒度）：`req:R09-040`
  -（可选）通用权限点：`datacenter:read`
  - 视角/范围：学校视角仅本校；集团视角可 `X-View-Scope` 切换（进入学校视角必须审计）。
- **User Stories**：
  - 场景 1：作为系统管理员/运维管理员，我希望能够完成“列表”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-LIST`
  - 验收用例：
    1. 权限校验：具备 `req:R09-040`（及必要通用权限点）才允许执行；无权限必须拒绝。
