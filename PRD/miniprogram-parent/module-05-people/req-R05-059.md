...
- **Requirement ID**：【R05-059】
- **Feature name**：家长信息
- **Source**：模块05：人员相关 15% > 学生管理 > 家长相关 > 信息填写 > 填写家庭相关信息 > 家长信息
- **Applicable Platform/Entry**：小程序（家长端） / 模块05 / 家长信息
- **User Roles & Permission Requirements**：
  - 可执行角色：家长/监护人
  - 权限点（最小粒度）：`req:R05-059`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为家长/监护人，我希望能够完成“家长信息”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-GENERAL`
  - 验收用例：
    1. 权限校验：具备 `req:R05-059`（及必要通用权限点）才允许执行；无权限必须拒绝。
