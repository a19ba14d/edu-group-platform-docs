...
- **Requirement ID**：【R05-062】
- **Feature name**：一对多家长管理
- **Source**：模块05：人员相关 15% > 学生管理 > 家长相关 > 相关人授权 > 一对多家长管理
- **Applicable Platform/Entry**：小程序（家长端） / 模块05 / 一对多家长管理
- **User Roles & Permission Requirements**：
  - 可执行角色：家长/监护人
  - 权限点（最小粒度）：`req:R05-062`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为家长/监护人，我希望能够完成“一对多家长管理”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-GENERAL`
  - 验收用例：
    1. 权限校验：具备 `req:R05-062`（及必要通用权限点）才允许执行；无权限必须拒绝。
...
- [【R05-063】统计功能](../../web-admin/module-05-people/req-R05-063.md)
