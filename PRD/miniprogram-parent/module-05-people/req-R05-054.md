...
- **Requirement ID**：【R05-054】
- **Feature name**：推送家长
- **Source**：模块05：人员相关 15% > 学生管理 > 学生信息 > 补充功能 > 健康 > 体温检测 > 报警信息推送 > 推送家长
- **Applicable Platform/Entry**：小程序（家长端） / 模块05 / 推送家长
- **User Roles & Permission Requirements**：
  - 可执行角色：家长/监护人
  - 权限点（最小粒度）：`req:R05-054`
  -（可选）通用权限点：`people:send`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为家长/监护人，我希望能够完成“推送家长”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-NOTIFY`
  - 验收用例：
    1. 权限校验：具备 `req:R05-054`（及必要通用权限点）才允许执行；无权限必须拒绝。
