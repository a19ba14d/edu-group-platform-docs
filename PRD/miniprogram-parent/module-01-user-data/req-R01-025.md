...
- **Requirement ID**：【R01-025】
- **Feature name**：OpenId 关联登录
- **Source**：模块01：用户数据 > 用户登录 > 家长及学生 > 小程序端 > OpenId 关联登录
- **Applicable Platform/Entry**：小程序（家长端） / 模块01 / OpenId 关联登录
- **User Roles & Permission Requirements**：
  - 可执行角色：家长/监护人
  - 权限点（最小粒度）：`req:R01-025`
  -（可选）通用权限点：`user:auth`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为家长/监护人，我希望能够完成“OpenId 关联登录”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-AUTH`
  - 验收用例：
    1. 权限校验：具备 `req:R01-025`（及必要通用权限点）才允许执行；无权限必须拒绝。
...
- **备注**：学生端与家长端均需支持 OpenId 关联登录；主归属家长端，学生端按入口复用。

## 相关需求（See also）
- [【R01-024】手机验证码登录](./req-R01-024.md)
- [【R01-016】地址等](./req-R01-016.md)
- [【R01-026】前端服务器方案](../../web-admin/module-01-user-data/req-R01-026.md)
