...
- **Requirement ID**：【R07-027】
- **Feature name**：笔记系统
- **Source**：模块07：在线学习 15% > 课程跟踪 > 笔记系统
- **Applicable Platform/Entry**：小程序（学生端） / 模块07 / 笔记系统
- **User Roles & Permission Requirements**：
  - 可执行角色：学生
  - 权限点（最小粒度）：`req:R07-027`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为学生，我希望能够完成“笔记系统”，以便在权限允许且 `school_id` 隔离范围内推进业务。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-GENERAL`
  - 验收用例：
    1. 权限校验：具备 `req:R07-027`（及必要通用权限点）才允许执行；无权限必须拒绝。
