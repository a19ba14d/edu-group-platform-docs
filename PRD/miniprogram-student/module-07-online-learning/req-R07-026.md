...
- **Requirement ID**：【R07-026】
- **Feature name**：学习评估
- **Source**：模块07：在线学习 15% > 课程跟踪 > 学习评估
- **Applicable Platform/Entry**：小程序（学生端） / 模块07 / 学习评估
- **User Roles & Permission Requirements**：
  - 可执行角色：学生
  - 权限点（最小粒度）：`req:R07-026`
  -（可选）通用权限点：`learning:report`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为学生，我希望能够完成“学习评估”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为学生，我希望查看指标/报表并在授权范围内切换汇总/单校视角，确保口径一致。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-REPORT`
  - 验收用例：
    1. 权限校验：具备 `req:R07-026`（及必要通用权限点）才允许执行；无权限必须拒绝。
    2. 租户隔离：尝试跨 school_id（或未授权 X-View-Scope）访问/操作必须失败；不得出现跨校可查/可改。
    3. 输入校验：UUID/必填字段/格式校验失败时应返回明确错误。
    4. 指标口径：同一指标在不同入口/视角下口径一致；结果可复现。
    5. 视角授权：集团汇总遵循 `X-View-Scope` 与授权范围；不得泄露未授权学校明细。
    6. 性能门禁：复杂报表满足附件3性能指标；明细列表分页且 `limit<=100`。

## 相关需求（See also）
- [【R07-024】我的课程](./req-R07-024.md)
