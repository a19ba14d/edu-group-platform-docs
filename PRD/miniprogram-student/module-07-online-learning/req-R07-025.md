...
- **Requirement ID**：【R07-025】
- **Feature name**：学习历史
- **Source**：模块07：在线学习 15% > 课程跟踪 > 学习历史
- **Applicable Platform/Entry**：小程序（学生端） / 模块07 / 学习历史
- **User Roles & Permission Requirements**：
  - 可执行角色：学生
  - 权限点（最小粒度）：`req:R07-025`
  -（可选）通用权限点：`learning:read`
  - 视角/范围：不涉及后台双视角切换；以绑定/所属学校 `school_id` 与身份关系为边界。
- **User Stories**：
  - 场景 1：作为学生，我希望能够完成“学习历史”，以便在权限允许且 `school_id` 隔离范围内推进业务。
  - 场景 2：作为学生，我希望能分页查看数据并按关键字/状态筛选，以便快速定位目标记录。
...
- **Acceptance Criteria（verifiable）**：
  - 验收模板：`T-LIST`
  - 验收用例：
    1. 权限校验：具备 `req:R07-025`（及必要通用权限点）才允许执行；无权限必须拒绝。
